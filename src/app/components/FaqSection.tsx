...
export default function FaqSection() {
  ...
  return (
    <section id="faq" className="py-24 px-6 lg:px-12 bg-white border-t border-slate-50">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="text-3xl lg:text-5xl font-black text-slate-900 leading-tight mb-4">
            {t[lang].faqH2}
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          {t[lang].faqData.map((faq: { q: string; a: string }, idx: number) => (
            ...
            >
              <button
                className={`w-full text-left px-7 py-6 flex items-center justify-between font-bold text-base lg:text-lg transition-colors ${openFaq === idx ? 'text-blue-900' : 'text-slate-900'
                  }`}
                onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
              >
                <span className="pr-4">{faq.q}</span>
                <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all ${openFaq === idx ? 'bg-blue-900 text-white rotate-180' : 'bg-slate-100 text-slate-400'
                  }`}>
