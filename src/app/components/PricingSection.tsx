  ...
  return (
    <section id="harga" className="py-24 px-6 lg:px-12 bg-white rounded-t-[3rem]">
      <div className="max-w-[1400px] mx-auto flex flex-col items-center">
        ...
        <div className="flex flex-wrap items-stretch justify-center gap-2 mb-10 w-full max-w-5xl p-1.5 bg-slate-50 rounded-2xl border border-slate-100 relative z-50">
          {pricingServiceGroups.map((group: any) => (
            <div key={group.id} className="relative grow md:flex-initial" ref={group.id === 'web' ? dropdownRef : null}>
              <button
                onClick={() => {
                  if (group.id === 'web') {
        ...
        {/* Pricing Cards - 20% Smaller Layout */}
        <div className="relative w-full">
          <motion.div
            ...
            className={`flex flex-nowrap overflow-x-auto snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:grid ${currentPricing.length === 1 ? 'lg:grid-cols-1 max-w-md mx-auto' :
              currentPricing.length === 2 ? 'lg:grid-cols-2 max-w-3xl mx-auto' :
                'lg:grid-cols-3 max-w-6xl mx-auto'
              } gap-6 lg:items-center w-full pb-10 pt-6 px-4`}
          >
            {currentPricing?.map((pkg: any, index: number) => (
              <motion.div
                ...
                className={`relative min-w-[260px] md:min-w-0 p-6 lg:p-7 rounded-4xl border transition-all duration-300 snap-center flex flex-col ${pkg.isPopular
